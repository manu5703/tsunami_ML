...
| Rows used | 2,000,000 (oversampled with 1% Gaussian jitter to match index scale) |
...
Training queries directly specify latitude/longitude ranges for the Bay Area combined with housing attribute ranges (income, rooms, value). Nearby queries use the `Place` lookup table to resolve Silicon Valley city names into tight lat/lon bounding boxes — geographic sub-clusters within the training region. Far queries apply the same lookup for SoCal cities, which lie entirely outside the trained region in the 9-dimensional feature space.
...
| Rows used | 2,000,000 (random subsample or oversample with 1% jitter) |
| Dimensions | 10 continuous columns |
| Lookup table | `Neighbourhood` — maps NYC neighborhood name to (Lat_min, Lat_max, Lon_min, Lon_max) |

**Note on year choice:** 2016 is the last year the TLC dataset included raw pickup/dropoff latitude and longitude. From 2017 onward TLC replaced coordinates with zone IDs, which cannot be used as numeric range features for the Tsunami index.
...
| Group | Zone | Neighborhoods | Key Filters | Approx. Selectivity |
|---|---|---|---|---|
| **Training** | Central Manhattan | Midtown, Chelsea, Gramercy | Lat/Lon tight boxes + fare, distance, tip ranges | ~2–4% per neighborhood |
| **Nearby (Group A)** | Surrounding Manhattan | Flatiron, Murray Hill, Hell's Kitchen, West Village, SoHo | Same trip-attribute filters | ~2–4% per neighborhood |
| **Far (Group B)** | Outer Boroughs | Flushing, Jamaica, JFK Airport, Coney Island, Fordham, Riverdale, Flatbush | Same trip-attribute filters | ~1–3% per neighborhood |

Training queries target three adjacent high-density pickup zones in central Manhattan using raw lat/lon BETWEEN ranges. Nearby queries use the `Neighbourhood` lookup table for adjacent Manhattan neighborhoods — geographically close to the training zone and sharing similar fare and distance distributions. Far queries target outer borough locations (Queens, Brooklyn, the Bronx) that are 10–20 km from the training zone with sparser pickup density.
...
| Dimensions | 10 continuous columns (first 10 features; binary wilderness/soil columns excluded) |
...
Krummholz is a geographic and ecological subset of HighAlpine. Tsunami is trained on the broader alpine zone and therefore builds highly precise partitions covering the entire alpine band, including the tighter Krummholz sub-cluster. Lowland terrain (Cottonwood/Willow, riparian valleys) sits at the opposite extreme of the elevation gradient, in coarse unoptimized partitions.

---

## How a Query Flows Through Tsunami's Components

The following describes the execution path for any query across all three datasets.
...
The SQL string is tokenized and the WHERE clause split on AND into individual conditions. Each condition is converted into a per-column numeric range constraint. For lookup keywords (Place, Neighbourhood, Zone), the name is resolved to column ranges before the Query object is assembled. Columns with no constraint receive [col_min, col_max] — unconstrained (full range).
...
The Tsunami index organises data in a recursive binary tree of axis-aligned partitions (built with gt_max_depth=4, giving up to 16 leaf nodes). Starting from the root, each internal node splits the data space along one dimension at a learned threshold. At each node the query bounding box is tested:

- **No overlap** — prune the entire subtree; zero rows scanned.
- **Full containment** — include all rows in the subtree without further descent.
- **Partial overlap** — recurse into both children.

Only the surviving leaf partitions proceed to the scan stage.

### Step 3 — AGD-Optimized Partition Boundaries

During index construction, Active Gradient Descent (AGD) iteratively shifts the GT split thresholds to minimize total rows scanned across all training queries. For the HighAlpine training workload (Elevation > 3200, Slope > 15), AGD concentrates several leaf partitions in the alpine band, making each leaf small and precise. The dominant subalpine zone (~65% of data) is covered by only a few large coarse leaves.

### Step 4 — Leaf Scan and Aggregation

For each surviving leaf partition, rows are checked against the exact query ranges and the aggregation (COUNT / AVG / SUM) is accumulated. The result includes:

- **value** — the aggregation result
- **n_scanned** — total rows inspected across selected leaves
- **n_matched** — rows satisfying all conditions

### Step 5 — Brute Force Baseline

`idx.brute_force(q)` scans all 2,000,000 rows sequentially with the same range filters. This is the baseline for speedup:
...
|---|---|---|---|
| **Nearby** (Krummholz — tight subset of training zone) | Query falls inside fine alpine partitions; few small leaves selected | Low (~5–10%) | High |
| **Far** (Lowland — opposite end of elevation range) | Query falls in coarse boundary leaves covering a wide elevation band | Higher (~15–30%) | Lower |

Both groups return a similar absolute number of matching rows (~3–10% of data), so the speedup difference is purely due to partition resolution — exactly the claimed benefit of workload-driven index construction in the Tsunami paper.
